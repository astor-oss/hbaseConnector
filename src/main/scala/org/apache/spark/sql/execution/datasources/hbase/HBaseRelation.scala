...
case class HBaseRelation(
  ...
  @transient val  hConf = {
    if (testConf) {
      logError(s"huzongxingTestConf==> " + SparkHBaseConf.testConf)
      SparkHBaseConf.conf
    } else {
      val conf = HBaseConfiguration.create
      //hBaseConfiguration.foreach(_.foreach(e => conf.set(e._1, e._2)))
      hBaseConfiguration.foreach(_.foreach(e => {logError("huzongxingConf====>"+ e._1 + "v:" + e._2)
        conf.set(e._1, e._2)}))
      conf
    }
  }

  logError(s"huzongxingConf==> ${hConf.toString} regions")
  logError("huzongxingConf---> zookeeper.znode.parent = " + hConf.get("zookeeper.znode.parent"))
  logError("huzongxingConf--> hbase.zookeeper.quorum = " + hConf.get("hbase.zookeeper.quorum"))
  logError("huzongxingConf--> HBaseConfiguration = " + hConf)
  println("huzongxingConf=>" + hConf.toString)
  hConf.set("hbase.zookeeper.quorum", "192.168.1.81,192.168.1.82,192.168.1.83")
  hConf.set("hbase.zookeeper.property.clientPort", "2181")
  hConf.set("zookeeper.znode.parent", "/hbase-unsecureTest")
  logError("huzongxingConf---> zookeeper.znode.parent = " + hConf.get("zookeeper.znode.parent"))
  logError("huzongxingConf--> hbase.zookeeper.quorum = " + hConf.get("hbase.zookeeper.quorum"))
  logError("huzongxingConf--> HBaseConfiguration = " + hConf)
